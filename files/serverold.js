 ...
 * Reads NERV_API and PUBLIC_URL from environment for Railway deployment
 */
...
// Accept all origins — Railway sets proper domain restrictions via firewall
app.use(cors({
...
// ─── ENV CONFIG ──────────────────────────────
// Locally:   NERV_API=http://127.0.0.1:3000/api  PUBLIC_URL=http://localhost:5000
// Railway:   NERV_API=https://nervhq.up.railway.app/api  PUBLIC_URL=https://aniwatch-bridge.up.railway.app
...
// Serve index.html and static assets from the same folder
...
app.get('/auth/mal/callback', async (req, res) => {
  ...
  try {
    ...
    if (!r.ok) { const err = await r.text(); console.error('[MAL] Token error:',err); return res.status(400).send('MAL auth failed.'); }
    const tokens = await r.json();
    const maxAge = tokens.expires_in || 3600;
    const secure = PUBLIC_URL.startsWith('https') ? '; Secure' : '';
    res.setHeader('Set-Cookie', [
      `mal_access_token=${tokens.access_token}; HttpOnly; SameSite=Lax; Max-Age=${maxAge}; Path=/${secure}`,
...
async function refreshToken(req, res) {
  ...
  try {
    const r = await fetchT(`${MAL_AUTH}/token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ client_id: MAL_CLIENT_ID, grant_type: 'refresh_token', refresh_token: refresh }).toString(),
    });
    if (!r.ok) return null;
    const tokens = await r.json();
    const secure = PUBLIC_URL.startsWith('https') ? '; Secure' : '';
...
║   ${PUBLIC_URL.padEnd(40)}║
║   NervHQ: ${NERV_API.padEnd(31)}║
╚══════════════════════════════════════════╝
`));
